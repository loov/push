...
func TestIsSysEx(t *testing.T) {
	tests := []struct {
		...
		{"mackie control xt", []byte{0x00, 0x00, 0x66, 0x15, 0x00}, true},
		{"logic control", []byte{0x00, 0x00, 0x66, 0x10, 0x00}, true},
		{"logic control xt", []byte{0x00, 0x00, 0x66, 0x11, 0x00}, true},
		{"wrong prefix", []byte{0x00, 0x00, 0x67, 0x14, 0x00}, false},
		{"wrong model", []byte{0x00, 0x00, 0x66, 0x99, 0x00}, false},
...
func TestClassifySysEx(t *testing.T) {
	tests := []struct {
		...
		{"meter mode", []byte{0x00, 0x00, 0x66, 0x14, 0x20, 0x03, 0x05}, SysExMeterMode},
...
func TestEncodeSerialReply(t *testing.T) {
	resp := EncodeSerialReply(ModelIDMackieControl, DefaultSerial)
	if resp[0] != 0xF0 {
	...
	if resp[4] != ModelIDMackieControl {
		t.Errorf("model ID = 0x%02X, want 0x%02X", resp[4], ModelIDMackieControl)
	}
	if resp[5] != 0x1B {
